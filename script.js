...
const projectText = {
  en: {
    tuni: {
      ...
      text: "A standalone case from the Tuni coffee machine project folder, including product stills and motion assets. This category stays inside the site instead of relying on an external ZCOOL jump.",
    },
    wigomat: {
      ...
      text: "A complete commercial detail-page case for a German coffee machine brand entering the Chinese market, combining brand introduction, scene photography, 3D rendering and refined e-commerce storytelling.",
    },
    bosch: {
      ...
      text: "AI-driven visual pages for BOSCH home appliance products, focused on product scenes, generated imagery, compositing and e-commerce landing-page execution.",
    },
    sondy: {
      ...
      text: "A long-form e-commerce detail page for SONDY vacuum cleaner products, using 3D product visuals and structured information blocks to explain selling points and product function.",
    },
    curvymoon: {
      ...
      text: "A full Amazon A+ visual package for a Curvymoon vacuum cleaner, including hero images, feature modules, large image blocks and product-angle storytelling.",
    },
    lieren: {
      ...
      brand: "ThundeRobot",
      role: "Visual concept, graphic design, 3D",
      text: "A visual poster case for a ThundeRobot gaming laptop, showing the process from early atmosphere sketches to final space-station compositions, transparent shells and explosive product graphics.",
    },
    peripheral: {
      ...
      text: "Three consumer-electronics product render sets for ThundeRobot peripherals, focused on material quality, product combinations and commercial hardware imagery.",
    },
    dishwasher: {
      ...
      text: "An AI compositing exercise around a side-opening dishwasher angle, rebuilding perspective and product structure for a more convincing e-commerce scene.",
    },
    g45: {
      ...
      text: "Product rendering for the ThundeRobot G45 game controller, focused on hardware material, structure, wiring details and clean commercial presentation.",
    },
    watch: {
      ...
      text: "A mechanical watch rendering study using a dark scene and metallic material language to highlight complex structure and premium product form.",
    },
    avata: {
      ...
      text: "A DJI Avata product visualization practice focused on the drone body, vertical composition, strong lighting and early 3D product presentation ability.",
    },
    keyboard: {
      ...
      text: "A keyboard modeling and rendering practice aimed at clean commercial product imagery, material control and white-background hardware presentation.",
    },
  },
  zh: {
    tuni: {
...
const dialogTag = dialog.querySelector(".eyebrow");
...
function getProjectCopy(project) {
  return {
    ...project,
    ...(projectText[currentLanguage]?.[project.id] || projectText.en[project.id] || {}),
  };
}
...
function applyLanguage() {
  ...
  document.querySelector(".hero h1")?.setAttribute("data-text", t("heroTitlePlain"));
  document.querySelector(".dialog-link").textContent = t("openZcool");
...
function setupHeroCarousel() {
  ...
  const switchHero = (mode = "auto") => {
    ...
    heroNextImage.src = heroPool[nextIndex];
    buildHeroSlices(heroPool[nextIndex]);
    heroVisual.classList.remove("is-switching");
    void heroVisual.offsetWidth;
    heroVisual.classList.add("is-switching");
    window.setTimeout(() => {
      heroCurrentImage.src = heroPool[nextIndex];
      heroIndex = nextIndex;
      heroVisual.classList.remove("is-switching");
...
function setupVideoReplay() {
  ...
  const restartVideo = (video) => {
    ...
    video.play().then(() => {
      ...
      video.currentTime = 0;
      video.play().then(() => {
        showreel.classList.add("needs-sound");
      }).catch(() => {
        showreel.classList.add("needs-sound");
      });
    });
  ...
  const unlockAudio = (restart = false) => {
    showreelVideos.forEach((video) => {
      ...
      if (video.getBoundingClientRect().top < window.innerHeight && video.getBoundingClientRect().bottom > 0) {
        video.play().then(() => {
          video.closest(".showreel")?.classList.remove("needs-sound");
        }).catch(() => {});
      }
    });
  };
  document.querySelector(".sound-unlock")?.addEventListener("click", () => unlockAudio(true));
      ...
      return `
        <article class="feature-card" data-project="${project.id}" data-reveal style="--reveal-delay: ${index * 55}ms">
          ...
          <div class="feature-content">
            <span>${projectNumber(index)} / ${getCategoryLabel(project.category)}</span>
            <h3>${copy.short}</h3>
            <p>${copy.brand} / ${copy.role}</p>
          </div>
        </article>
      `;
    })
...
function renderSlide() {
  ...
  const autoButton = dialog.querySelector(".case-auto");
  if (!stage) return;
  ...
  if (autoButton) {
    autoButton.classList.toggle("active", caseAuto);
    autoButton.setAttribute("aria-pressed", String(caseAuto));
...
function startCaseAuto() {
  stopCaseAuto();
  if (!caseAuto || activeGallery.length < 2 || window.matchMedia("(prefers-reduced-motion: reduce)").matches) return;
...
function bindEvents() {
  document.body.addEventListener("click", (event) => {
    ...
    if (langButton) {
      currentLanguage = langButton.dataset.lang;
    ...
    if (event.target.closest(".case-stage")) {
      stepSlide(1);
      startCaseAuto();
    }
    if (event.target.closest(".case-prev")) {
      stepSlide(-1);
      startCaseAuto();
    }
    if (event.target.closest(".case-next")) {
      stepSlide(1);
      startCaseAuto();
    }
  ...
  indexList.addEventListener("focusin", (event) => {
    const row = event.target.closest(".index-row");
    if (row) updateIndexPreview(row.dataset.project);
  });
  ...
  dialog.addEventListener("wheel", (event) => {
    ...
    const stage = event.target.closest(".case-stage");
    if (stage) {
      const now = performance.now();
      if (now - lastCaseWheel > 520) {
...
function updateWorkScroller() {
  ...
  cards.forEach((card, index) => {
    ...
    card.style.setProperty("--card-tilt", Math.max(-1, Math.min(1, delta)).toFixed(3));
